...
##############################################################################################################################
# This script simulate an interuppted single cell generations and perform post translation modifications in Monte Carlo cycles
##############################################################################################################################
...
if __name__ == "__main__":
    ...
    cycles  = 1000     # Total no of monte carlo cycle
    ...
    dump = sim.add_dump('stage1', 'all', 'xyz', every, fileext='.xyz')
    equifn = main_outname + '_total_dynamics_stage1'
    sim.run_sim(equifn,eq_run, processors)
    sim.save_state(equifn + '.state')

    # write the sequence to file
   
    seqfn = main_outname + '.seq_before_interruption'
    with open(seqfn,'w') as f:
        f.write(getseq(sim.elements.get_atoms_of_types(['N','M']))+'\n')

 
    ###########################################################################
    ########################## Monte Carlo loops (Stage- 1) ###################
    ###########################################################################
    # for periodic boundary condition
    # performing only the methylation reactions. No demethylation have been considered.

    stage1_run=0
    no_of_initial_runs=200

    for cc in range(no_of_initial_runs):

        cycle_outname = main_outname + f'_cycle{cc+1}'

        sim.run_sim(cycle_outname,steps, processors)

        sim.elements.dump2xyz(main_outname+'_total_dynamics_stage1',append=True, timestep=eq_run+cc*steps)
        ...
        with open(seqfn,'a') as f:
            f.write(getseq(sim.elements.get_atoms_of_types(['N','M']))+'\n')
        stage1_run+=1
        if (len(sim.elements.get_atoms_of_types(['M']))>=110):
            break


    # ------------- inetrrupt the interaction--------

    sim.types.add_pair("M","hp1","lj/cut",1.12246204,[1., 1.0])
    sim.remove_dump('stage1')
    dump = sim.add_dump('stage2', 'all', 'xyz', every, fileext='.xyz')
    equifn = main_outname + '_total_dynamics_stage2'
    sim.run_sim(equifn,5000000, processors)
    sim.save_state(equifn + '.state')

    #-------------- turn on the interaction--------

    sim.types.add_pair("M","hp1","lj/cut",2.5,[eps_nuc, 1.0])
    sim.remove_dump('stage2')
    dump = sim.add_dump('stage3', 'all', 'xyz', every, fileext='.xyz')
    equifn = main_outname + '_total_dynamics_stage3'

    # A conditional check for droplet formation could be inserted here like before

    sim.run_sim(equifn,10000000, processors)
    sim.save_state(equifn + '.state')

    # write the sequence to file

    seqfn = main_outname + '.seq_after_interruption'
    with open(seqfn,'w') as f:
        f.write(getseq(sim.elements.get_atoms_of_types(['N','M']))+'\n')
 
    ###########################################################################
    ########################## Monte Carlo loops (Stage- 2) ###################
    ###########################################################################

    # for periodic boundary condition
    # performing only the methylation reactions. No demethylation have been considered.

    for cc in range(stage1_run, cycles):

        #file_name.write(str(cc+1)+' ')

        cycle_outname = main_outname + f'_cycle{cc+1}'

        sim.run_sim(cycle_outname,steps, processors)

        sim.elements.dump2xyz(main_outname+'_total_dynamics_stage3',append=True, timestep=5000000+cc*steps)
